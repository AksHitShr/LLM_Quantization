...
for name, param in model.named_parameters():
    ...
    quantized_params[name] = param_int8
    quantization_coeffs[name] = {"scale": scale}

for name, param in model.named_parameters():
    param.data = quantized_params[name].to(param.data.device)

save_path="GPT2_FullQuant.pth"
torch.save({
    "model_state_dict": model.state_dict(),
    "quantization_config": quantization_coeffs,
}, save_path)
...
for name, param in model.named_parameters():
    param.data = dequantize_param(name).to(param.data.device)
